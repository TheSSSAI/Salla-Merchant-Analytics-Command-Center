...
# for the production environment. These values are consumed by the CI/CD pipeline to inject 
...
output "clickhouse_connection_string" {
  description = "The secure, TLS-enabled connection string for the ClickHouse Cloud production instance. Used by the data pipeline and analytical services."
...
output "redis_endpoint" {
  description = "The endpoint URL (host:port) for the Upstash Redis production instance."
  value       = module.session_cache.redis_endpoint
}

output "redis_password" {
  description = "The access password for the Upstash Redis production instance."
...
output "qstash_token" {
  description = "The authentication token for publishing messages to the QStash production topic. Used by the CDC pipeline."
  value       = module.event_bus.qstash_token
  sensitive   = true
}

output "qstash_current_signing_key" {
  description = "The current signing key used to verify the integrity of incoming webhooks from QStash in the production environment."
  value       = module.event_bus.qstash_current_signing_key
  sensitive   = true
}

output "qstash_next_signing_key" {
  description = "The next signing key for QStash webhooks, used for key rotation support in the production environment."
